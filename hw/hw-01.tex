...
\begin{exercise}{P.1}
Let $p\in S_6$ be the permutation written in `one-line notation' as
\[
p=[4,1,6,2,5,3].
\]
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item Express $p$ as a product of exactly $\ell(p)$ simple
        transpositions.
\end{enumerate}
\end{exercise}

\begin{exercise}{P.2}
Let $1\leq a<b\leq n$, and consider the transposition
\[
\tau=(a\ b)\in S_n.
\]
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \item Prove that
        \[
        \operatorname{inv}(\tau)=2(b-a)-1.
        \]
  \item Deduce that $\ell(\tau)=2(b-a)-1$.
  \item Use this result to explain why two permutations with the same cycle
        type need not have the same Coxeter length.
\end{enumerate}
\end{exercise}
