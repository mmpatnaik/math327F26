...
\begin{solution}
Choose $k>0$ such that $A^k=0$, and set
\[
...
\begin{solution}
Suppose first that $I_m-AB$ is invertible, and write
$X=(I_m-AB)^{-1}$.  We claim that
\[
  Y=I_n+BXA
\]
is the inverse of $I_n-BA$.  Indeed,
\begin{align*}
 (I_n-BA)Y
 &=I_n-BA+BXA-BABXA\\
...
\begin{enumerate}[label=\textbf{(\alph*)}]
  ...
  \]
  Also $a_i\in H=d\Z$, so $d\mid a_i$ for every $i$.  If $c$ is any
