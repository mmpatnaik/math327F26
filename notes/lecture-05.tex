...
\newtcolorbox{warning}{enhanced,breakable,colback=warningyellow,colframe=warningborder,coltitle=darkgray,fonttitle=\bfseries,title={Warning},boxrule=0.7pt,arc=2pt,left=6pt,right=6pt,top=5pt,bottom=5pt,before skip=0.8em,after skip=0.8em}
...
\begin{theorem}
	\begin{enumerate}
		...
		\item The smallest number of simple transpositions needed to write $p$ as in \eqref{p:decom} is called the length of $p$, denoted $\ell(p)$, and is equal to the number of inversions in $p$. That is,
		\be
		\ell(p)=\#\inv(p),
		\qquad
		\inv(p) := \{ (i,j) \mid 1 \leq i < j \leq n,\ p(i) > p(j) \}.
		\ee
	\end{enumerate}
\end{theorem}

The key to proving property (2) was the following lemma, which I slightly misstated last time. The statement has been corrected in the notes, and a proof is now included.
...
Thus multiplying by a simple transposition changes the number of inversions by exactly one (an odd number). 

\begin{define} A permutation $p \in S_n$ to be \textit{even} if it can be written as a product of an even number of simple transpositions. \end{define}

\begin{warning} 
Why is this a well-defined concept ? In other words, can a permutation be written both as an even and an odd number of simple transpositions ? \end{warning}

No! Start with an easy case. Can we write the trivial permutation using an odd number of simple transpositions, i.e. $(1) = s_{i_1} \cdots s_{i_{2m+1}}$ ? By the Lemma we can think of the right hand side as starting from $p=(1)$ and then applying a sequence of $2m+1$ simple transpositions, each of which changes the number of inversions by $\pm 1$. So the number of inversions would have to be odd (imagine starting at $0$ and adding/subtracting $1$ 7 times... ). On the other hand, this number of inversions must also be equal to the number of inversions of $(0)$ so it has to be $0$, a contradiction.  

Now for the general case: say $p = s_{i_1} \ldots s_{i_k}$ with $k$ even and also $p = s_{j_1} \cdots s_{j_d}$ with $d$ odd.  Then $p^{-1} = s_{j_d} \cdots s_{j_1}$ (Why? ) So then \be (1) = p p^{-1} = s_{i_1} \cdots s_{i_k} \, s_{j_d} \cdots s_{j_1}. \ee So by what we just said $d+k$ is even, which means that $d$ and $k$ have the same parity.
 
 
 Denote the set of even permutations by
\be
A_n := \{ p \in S_n \mid p \text{ is even} \}.
\ee
Equivalently, $p\in A_n$ if and only if $\ell(p)$ is even.

\begin{proposition} $A_n$ is a subgroup of $S_n$ \end{proposition}
\begin{proof} Since $S_n$ is finite, it suffices to check that $A_n$ contains the identity and is closed under multiplication. The identity is a product of zero simple transpositions, so it lies in $A_n$. If $p,q\in A_n$, choose expressions for $p$ and $q$ involving even numbers of simple transpositions. Concatenating these expressions gives an expression for $pq$ involving an even number of simple transpositions. Hence $pq\in A_n$. \end{proof}

We will come back and give a different proof of this result a bit later.

\begin{exercise}For $n\geq2$, how many even permutations are there? \end{exercise}
...
The table below gives the multiplication in $S_3$. Its entry in row $g$ and
column $h$ is $gh$, so $h$ is applied first.
\be
\begin{array}{c|cccccc}
