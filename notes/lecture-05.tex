...
Thus multiplying by a simple transposition changes the number of inversions by exactly one and hence reverses its parity. Define a permutation $p \in S_n$ to be \textit{even} if it can be written as a product of an even number of simple transpositions. This is a well-defined concept. Indeed, if $p$ has such an expression, then $\#\inv(p)$ is even. If it could also be written as a product of an odd number of simple transpositions, then the lemma would imply that $\#\inv(p)$ is odd, a contradiction. Denote the set of even permutations by
\be
